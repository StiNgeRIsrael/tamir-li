...
type ToolIconGridProps = {
  ...
  title: string;
  className?: string;
  /** "launcher" = native app feel: 2-column, denser tiles. */
  variant?: ToolIconGridVariant;
  /** Skip the section heading (parent already shows one). */
  hideTitle?: boolean;
};
...
const LAUNCHER_CARD_WIDTH = "w-[calc(50%-0.375rem)] max-w-none sm:w-[calc(33.333%-0.5rem)]";
...
export function ToolIconGrid({
  ...
  return (
    <section className={cn(isLauncher ? "space-y-2" : "space-y-5", className)}>
      {!hideTitle && (
        <h2
          className={cn(
            "font-bold text-foreground",
            isLauncher ? "text-start text-[13px]" : "text-center text-base sm:text-lg"
          )}
        >
          {title}
        </h2>
      )}
      <div className={cn("flex flex-wrap justify-center", isLauncher ? "gap-2.5" : "gap-3")}>
        {tools.map((tool) => {
            ...
            <>
              <div
                className={cn(
                  ...
                  isLauncher ? "h-12 w-12 rounded-xl" : "h-12 w-12 rounded-lg",
                  accentStyle.well,
              ...
              >
                <img
                  src={icon}
                  alt=""
                  width={isLauncher ? 40 : 44}
                  height={isLauncher ? 40 : 44}
                  className={cn(
                    "object-contain drop-shadow-sm",
                    isLauncher ? "h-10 w-10" : "h-11 w-11"
                  )}
                  loading="lazy"
                  decoding="async"
                />
              ...
              <span
                className={cn(
                  ...
                  isLauncher ? "mt-1.5 text-[11px]" : "mt-2.5 text-xs sm:text-sm",
              ...
              {!functional && (
                <span className="absolute top-1 end-1 z-10">
                  <ToolSoonBadge />
                </span>
              )}
              {tool.premium && (
                <span
                  className="absolute top-1 start-1 z-10 flex h-4 w-4 items-center justify-center rounded-full bg-premium/20 ring-1 ring-premium/35"
                  aria-hidden
                >
                  <Crown className="h-2.5 w-2.5 text-premium" strokeWidth={2.25} />
                </span>
              )}
            </>
          );
          ...
          const cardClass = cn(
            ...
            isLauncher ? "native-launcher-tile px-2 pb-2.5 pt-3" : "min-h-[7.75rem] px-2 pb-3 pt-5",
