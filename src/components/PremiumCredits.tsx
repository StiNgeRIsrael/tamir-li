...
interface CreditPackage { id: string; amount: number; price: number; pricePerImage: number; popular?: boolean; savings?: string; }
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        {packages.map((pkg) => (
          <div key={pkg.id} className={`relative border rounded-xl p-4 space-y-3 transition-all hover:shadow-md ${pkg.popular ? "border-premium bg-premium/5 shadow-sm" : "border-border bg-card"}`}>
            ...
            <Button className={`w-full font-bold ${pkg.popular ? "bg-premium text-premium-foreground hover:bg-premium/90" : "bg-primary text-primary-foreground hover:bg-primary/90"}`} size="sm">
