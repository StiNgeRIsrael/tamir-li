...
import { siteUrl, DEFAULT_OG_IMAGE, absoluteImageUrl, categoryOgImage, getOgImageDimensions } from "@/lib/site";
...
interface SEOHeadProps {
  ...
  siteName?: string;
  jsonLd?: object;
  /** When true, emit noindex,follow (e.g. coming-soon tool pages). */
  noindex?: boolean;
  /** Blog routes: only he-IL + x-default hreflang alternates. */
  hebrewOnlyHreflang?: boolean;
}
...
function isBlogBasePath(basePath: string): boolean {
  return basePath === "/blog" || basePath.startsWith("/blog/");
}
...
export function SEOHead({
  ...
  const blogHreflangOnly = hebrewOnlyHreflang ?? isBlogBasePath(basePath);
  const hreflangLocales: Locale[] = blogHreflangOnly ? ["he"] : [...LOCALES];
  ...
  const imageDims = getOgImageDimensions(imagePath);
  ...
  useEffect(() => {
    ...
    setMeta("og:image:width", String(imageDims.width), true);
    setMeta("og:image:height", String(imageDims.height), true);
    ...
    hreflangLocales.forEach((loc: Locale) => {
    ...
    if (jsonLd) {
      document.querySelectorAll('script[id^="static-json-ld-"]').forEach((el) => el.remove());
