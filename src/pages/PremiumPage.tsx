...
import {
  ...
  ImageIcon, Gauge, FileStack, Headphones, ChevronDown, ShieldCheck, X,
...
export default function PremiumPage() {
  ...
  const { checkout, checkoutLoading, refetch, captureOrder, isPremium } = useSubscription();
  ...
  const handleExitIntent = useCallback((e: MouseEvent) => {
    ...
    trackEvent(ANALYTICS_EVENTS.VIEW_PROMOTION, {
      ...
      promotion_name: isYearly ? "premium_yearly_exit" : "premium_monthly_exit",
        ...
        <section className="text-center space-y-6 animate-fade-in">
          ...
          <div className="max-w-sm mx-auto bg-card border-2 border-premium rounded-2xl p-6 shadow-lg shadow-premium/10 space-y-4">
            ...
            {isYearly && yearlyBestValue && (
              ...
              <p className="text-xs font-bold text-premium uppercase tracking-wide">{yearlyBestValue}</p>
          ...
          <div className="bg-card border border-border rounded-2xl overflow-hidden min-w-[300px]">
            ...
            {rows.map((row, i: number) => (
              ...
              >
                ...
                <div className={`p-2.5 sm:p-4 font-medium text-foreground ${row.highlight ? "font-semibold" : ""}`}>

                  {row.feature}

                </div>

                <div className="p-2.5 sm:p-4 text-center text-muted-foreground">{renderFreeCell(row)}</div>

                <div className={`p-2.5 sm:p-4 text-center font-semibold text-premium bg-premium/5 ${row.highlight ? "text-base sm:text-sm" : ""}`}>
        ...
        <DialogContent className="sm:max-w-md border-premium/20">
          ...
          <DialogHeader>
            ...
            <DialogDescription className="text-start pt-1 leading-relaxed">
              ...
              {exitIntent?.body ?? "Yearly Premium saves 20% — unlimited conversions, no ads, bigger files."}
